...
string getQueueName(int queue) {
    ...
    case 1: return "Q1 (Priority)";
    case 2: return "Q2 (SJF)";
...
void mlfqScheduler(Process processes[], int numProcesses, int quantumQ0) {
    ...
    queue<Process> q0;
    priority_queue<Process, vector<Process>, PriorityComparator> q1;
    priority_queue<Process, vector<Process>, SJFComparator> q2;
    ...
    while (completedProcesses < numProcesses) {
        ...
        if (!q0.empty()) {
            ...
            else if (preempt || p.priority > 5) {
                p.currentQueue = 2;
                p.queueWaitTime = 0;
                q2.push(p);
                cout << "  P" << p.pid << " demoted to " << getQueueName(2) << " (remaining time: " << p.remainingTime << ")\n";
            }
            else {
                p.queueWaitTime = 0;
                q1.push(p);
                cout << "  P" << p.pid << " reinserted into " << getQueueName(1) << " (remaining time: " << p.remainingTime << ")\n";
            }
            processes[p.pid - 1] = p;
        }
        else if (!q2.empty()) {
    ...
    cout << "\nProcess Details:\n";
    cout << "PID\tArrival\tBurst\tPriority\tCompletion\tWaiting\tTurnaround\tResponse\n";
    for (int i = 0; i < numProcesses; i++) {
        cout << processes[i].pid << "\t" << processes[i].arrivalTime << "\t" << processes[i].burstTime << "\t" << processes[i].priority << "\t\t"
            << processes[i].completionTime << "\t\t" << processes[i].waitingTime << "\t" << processes[i].turnaroundTime << "\t\t" << processes[i].responseTime << "\n";
